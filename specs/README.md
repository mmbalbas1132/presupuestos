...
| [005-cerrar-sesion](./005-cerrar-sesion/spec.md) | Control "Cerrar sesión" en la navegación principal, que reutiliza la función `cerrarSesion()` ya existente: limpia el estado local y redirige a `/acceso` tanto si el cierre de sesión tiene éxito como si falla, mostrando en ese caso un aviso no bloqueante. | ✅ Implementada (12/12 tareas) |
